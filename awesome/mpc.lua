...
function mpc.new(host, port, password, error_handler, ...)
    ...
    if not password then host, password = parse_password(host) end
...
function mpc:_connect()
    ...
    self._conn, self._output, self._input = conn, output,
                                            Gio.DataInputStream.new(input)
    ...
    local do_read
    do_read = function()
        self._input:read_line_async(GLib.PRIORITY_DEFAULT, nil,
                                    function(obj, res)
...
function mpc:send(...)
    ...
    self:_stop_idle()
    for i = 1, #args, 2 do self:_send(args[i], args[i + 1]) end
    self:_start_idle()
end
